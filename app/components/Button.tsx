...
export function Button(props: ButtonProps) {
  ...
  function $viewStyle({ pressed }:any) {
    return [
      $viewPresets[preset],
      $viewStyleOverride,
  ...
  function $textStyle({ pressed }:any) {
    return [
      $textPresets[preset],
      $textStyleOverride,
  ...
  return pending ?( 
    ...
    (<Pressable disabled={disable} style={$viewStyle} accessibilityRole="button" {...rest}>
      {(state) => (
        <>
...
const $Overlay:StyleProp<ViewStyle>={
  position: 'absolute', // Position the overlay on top of the content
  top: 0,
  left: 0,
  right: 0,
  bottom: 0,
  backgroundColor: 'rgba(255, 255, 255, 0.25)', // Semi-transparent white
  zIndex: 0, // Ensures overlay is below the content above
}
